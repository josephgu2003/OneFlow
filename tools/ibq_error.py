...
from ezflow.models.ibqgan import IBQ
...
from ezflow.utils.invert_flow import pad_flow
...
def vq_error(model, dataloader, device, metric_fn, flow_scale=1.0, pad_divisor=1):
    ...
    with torch.no_grad():
        ...
        for i, (inp, target) in tqdm(enumerate(dataloader)):
            ...
            flow_gt = target['flow_gt'] / 40
            flow_gt = pad_flow(flow_gt)
            outputs, _ = model(flow_gt)
            output = outputs[:, :2] * 40
...
if __name__ == "__main__":
    import yaml 
    with open('configs/models/ibq.yaml', 'r') as file:
        config = yaml.safe_load(file)
        
    model = load_vqgan_new(config, 'IBQ', ckpt_path='./pretrained_models/imagenet256_16384.ckpt') 
